...
public static function EstatusAprovadoOuReprovado9Classe(){
...
foreach ($dados as $disciplina) {
    ...
    if ($disciplina['TipoNome'] === 'gerais') {
        if ($disciplina['nomeDisciplina'] === 'LÍNGUA PORTUGUESA' || $disciplina['nomeDisciplina'] === 'MATEMÁTICA') {
            // LÍNGUA PORTUGUESA e MATEMÁTICA não contam como negativas
            continue;
        }
...
if ($negativasGerais <= 2) {
    echo "Transita para o próximo ano";
} elseif ($negativasGerais == 3 && $negativasEspecificas == 0) {
    echo "Vai ao recurso";
} else {
    echo "Reprovado";
}
